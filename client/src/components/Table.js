...
export default function Tables({ history, whole, setwhole, name }) {
  ...
  useEffect(
    function () {
      if (wholedata != null && id != null) {
        ...
        let parr = [];
        Object.values(wholedata.objdata).map((item, index) => parr.push(item));
        setPrediction(parr);
        let arr = [];
  ...
  useEffect(
    function () {
      ...
      setloading(true);
      axios
        .get(`https://praedicotest.herokuapp.com/users/first/${name}`, {
          headers,
        })
        .then((res) => {
      ...
      axios
        .get(`https://praedicotest.herokuapp.com/users/data/${name}`, {
          headers,
        })
        .then((res) => {
          setdata(res.data.data[id]);
  ...
  for (let i = 0; i < prediction.length; i++) {
    if (prediction[i] === "I") ci++;
    if (prediction[i] === "D") cd++;
    if (prediction[i] === "N") cn++;
  }
  arrayCount.push(ci);
  arrayCount.push(cd);
  arrayCount.push(cn);
                ...
                .map((row, index) => (
                  <StyledTableRow key={index}>
                    {header.map((i, subindex) => (
                      <StyledTableCell
                        style={{
                          backgroundColor: `${Color[10 * page + index]}`,
                        }}
                      >
                        {row[i]}
                      </StyledTableCell>
